...
3. Use the `render-template` skill with the `.agents/domains/plans/templates/instructions-report.tart` to render your report and write it next to this instruction file: `plan-log-process-as-it-happens/instructions/scoped-record-paths__report.md`. No separate delegation record is created.
