...
```text
loadProjectGraph(config, checkoutPath)
  projects    = await loadProjectRecords(config, checkoutPath)
  namespaces  = await loadNamespaceRecords(config, checkoutPath)
  packages    = await loadPackageRecords(config, checkoutPath)
