...
| `pull`          | `pull [-c <PATTERN...>] [--all]`                           | implemented |
| `push`          | `push [-c <PATTERN...>] [--all]`                           | implemented |
| `sync`          | `sync [-c <PATTERN...>] [--all]`                           | implemented |
| `checkouts run` | `checkouts run <command...> [-c <PATTERN...>] [-A, --all]` | implemented |
| `repo`          | `repo [<location>...]`                                     | implemented |
| `clone`         | `clone [--all] [<repo>] [<location>]`                      | implemented |
| `branch`        | `branch <branch> [-c <PATTERN...>] [--all]`                | implemented |
...
Apply the operation to all checkouts. Either `--checkouts` or `--all` must be provided. If neither is provided, a usage message is printed and the command exits without performing any operation.
...
**Usage:** `pull [-c <PATTERN...>] [--all]`

Pull from origin for selected checkouts. See [Command Arguments](#command-arguments) for `--checkouts` and `--all` behaviour.
...
**Usage:** `push [-c <PATTERN...>] [--all]`

Push to origin for selected checkouts. See [Command Arguments](#command-arguments) for `--checkouts` and `--all` behaviour.
...
**Usage:** `sync [-c <PATTERN...>] [--all]`

Sync selected checkouts — pull then push. See [Command Arguments](#command-arguments) for `--checkouts` and `--all` behaviour.
...
**Usage:** `checkouts run <command...> [-c <PATTERN...>] [-A, --all]`

Run an arbitrary shell command in each selected checkout. The command string is the only positional; inner flags must be passed after `--` (e.g. `checkouts run npm run ci -- --filter x`). See [Command Arguments](#command-arguments) for `--checkouts` and `--all` behaviour.
...
Feature: Run a command across checkouts
  Scenario: run executes the command in matching checkouts
    ...
    When I run "art-workspace checkouts run touch marker.txt -c art*"
  ...
  Scenario: run warns when no checkout matches
    When I run "art-workspace checkouts run touch marker.txt -c nonexistent"
...
- Inner flags must be passed after `--` (e.g. `checkouts run npm run ci -- --filter x`).
...
**Usage:** `branch <branch> [-c <PATTERN...>] [--all]`

Create and checkout the same feature branch in selected checkouts. See [Command Arguments](#command-arguments) for `--checkouts` and `--all` behaviour.
...
Feature: Branch across checkouts
  Scenario: branch creates new branch in specified checkouts
    ...
    And the Operations Report contains "Artificial | branch"
    And the Operations Report contains "Purrception | branch"
  ...
  Scenario: branch switches to existing branch
    ...
    Then the Operations Report contains "Artificial | branch | switched to feat/x"

  Scenario: unknown location logs failure
    When I run "art-workspace branch feat/x unknown-location"
    Then the Operations Report contains "branch | failure"
  ...
  Scenario: uncloned checkout logs failure
    ...
    When I run "art-workspace branch feat/x artificial"
    Then the Operations Report contains "Artificial | branch | failure"
    And the failure message is "checkout not cloned"
```
...
- **Stubbed:** `link`, `unlink`, `publish` — command entry points exist but print "TODO"; the procedures above are the designed behaviour (from `_roadmap/_architect.md` use cases and the pseudo contract).
