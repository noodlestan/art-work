...
| Package record (`_records/package.art`) | COMPLETED | Added package record for Package: Lib Records (`@art-cli/lib-records`) at `libs/records/_records/package.art` |
| Workspace resolution | COMPLETED | Root `package-lock.json` updated with workspace `@art-cli/lib-records` |
...
- `libs/records/package.json` — Package manifest for `@art-cli/lib-records`.
