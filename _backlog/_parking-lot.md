...
- **ScanCheckoutState should not do git checks in dirs without .git** — Some scan heuristics in `cli/work/src/private/scan/scanCheckoutState.ts` inherit state from parent dir when `.git` not present. Exampole. Create `hasGitDir` check, bypass others (exactly which?) when `.git` not present. Create `createGitDirState(hasGit)` and add before anything else.
- **Fix `--version` showing stale version** — `art-work-cli -- --version` shows `0.0.9` but package is at `0.0.14`. Investigate why version is not being read from package.json correctly.
